	...
	<c:set var="i" value="0" />
		<c:forEach var="event" items="${list }" >
			...
			<tr class="ct_list_pop">
				...
				<td align="center">${event.product.prodNo }
				<td></td>
				
				<td align="left">
					<a href="/product/updateProduct?prodNo=${event.product.prodNo }">${event.product.prodName }</a>
				</td>
				<td></td>
				...
				<td align="left">
					...
					${event.product.checkEvent }
					<c:if test="${event.product.checkEvent == 0 }" >
						할인안함&nbsp;&nbsp;&nbsp;
						<a href="/product/getProduct?prodNo=${event.product.prodNo }&menu=discount">할인적용</a>
					</c:if>
					<c:if test="${event.product.checkEvent == 1 }" >	
						<c:if test="${ start > today }" >
							할인예정&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	
							<a href="/product/getProduct?prodNo=${event.product.prodNo }&menu=discount">할인취소</a>
						</c:if>
						
						<c:if test="${ start <= today }" >
							할인중&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;	
							<a href="/getProduct.do?prodNo=${event.product.prodNo }&menu=discount">할인취소</a>
						</c:if>
