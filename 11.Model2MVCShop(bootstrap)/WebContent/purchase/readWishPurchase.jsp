...
<head>
	...
	$(function() {
		$( ".btn-primary:contains('수정')" ).on("click" , function() {
			...
			self.location = "/purchase/updateWishPurchase?tranNo="+tranNo;
		});
		...
		$(".btn-primary:contains('구매확정')").on("click", function(){
			var tranCount = ${purchase.tranCount};
			var prodCount = ${purchase.purchaseProd.prodCount};
			var tranNo = ${purchase.tranNo};
			
			if ( tranCount <= prodCount ) {
				self.location = "/purchase/checkPurchase?tranNo="+tranNo;
			}
			else {
				alert("현재 상품 수량은 " + prodCount + "개 입니다.");
				return false;
			}
...
<body>
	...
	<div class="container">
		<dlv class="page-header">
			<h3 class="text-info">장바구니 조회</h3>
			<h5 class="text-muted">장바구니 목록의 상세정보를 <strong class="text-danger">확인 </strong>할 수 있습니다.</h5>
		</dlv>
		...
		<div class="row">
			...
			<div class="col-xs-8 col-md-4" id="product">${purchase.tranCount }</div>
		</div>
		...
		<div class="row">
	  		<div class="col-md-12 text-center ">
	  			<button type="button" class="btn btn-primary">구매확정</button>	
	  			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
