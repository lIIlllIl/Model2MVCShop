...
<head>
	...
	$( function() {
		...
		$("h4[name='saleCheck']").on("click", function(){
			var prodNo = $(this).children("input[name='prodNo']").val();
 	 						...
 	 						success : function( JSONData, status ) {
 	 							...
 	 							for ( count = 0; count < JSONData.list.length; count++) { 
									...
									var image = '';
 	 								...
 	 								if ( JSONData.list[count].product.fileName != null ) {
 	 									image = JSONData.list[count].product.fileName.split("/")[0];
 	 								}
 	 								...
 	 								if ( JSONData.list[count].startDate != null ) {
 	 									display += "<h4>" + JSONData.list[count].startDate + " ~ " + JSONData.list[count].endDate;
 	 								}
 	 								if ( JSONData.list[count].startDate == null ) {
 	 									display += "<h4>&nbsp;</h4>"
 	 								}
				...
				<div class="col-xs-4 col-md-4">
					<div class="thumbnail">
						...
						<div class="caption">
							...
							<br/>
							<br/>
							<h4><p><span class="glyphicon glyphicon-asterisk"></span>&nbsp;&nbsp;&nbsp;${event.product.prodDetail}</p></h4>
							<h4><p><span class="glyphicon glyphicon-asterisk"></span>&nbsp;&nbsp;&nbsp;가격 : <fmt:formatNumber value="${event.product.price }" pattern="#,###" /></p></h4>
							<h4><p><span class="glyphicon glyphicon-asterisk"></span>&nbsp;&nbsp;&nbsp;판매수량 : ${event.product.prodCount} 개</p></h4>
							<fmt:parseDate var="dateString" value="${event.product.manuDate }" pattern="yyyyMMdd" />
							<h4><p><span class="glyphicon glyphicon-asterisk"></span>&nbsp;&nbsp;&nbsp;제조일자 : <fmt:formatDate value="${dateString}" pattern="yyyy-MM-dd" /></p></h4>
							<br/>
							<br/>
							...
							<c:if test="${event.product.checkEvent == 0 }" >
								할인안함
							</c:if>
