...
<script type="text/javascript">
  ...
  function nullCheck(value){ if( value == "" || value == null || value == undefined || ( value != null && typeof value == "object" && !Object.keys(value).length ) ){ return true }else{ return false } };

 	$(function(){

 		var addr = '';
 		
 		if ( ! nullCheck(opener.$("input[name='divyAddr']").val()) ) {
 			addr = opener.$("input[name='divyAddr']").val();
 		}
 		else {
 			addr = $("input[name='witch']").val();
 		}
		...
		$("input[name='receiverAddr']").focus();
 		...
 		$("span:contains('적용')").on("click" , function() {
 			
 			if(opener) {
 				opener.$("input[name='divyAddr']").val($("input[name='witch']").val());
				opener.$("input[name='divyAddr']").focus();
 			}
