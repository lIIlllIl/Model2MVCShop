...
//		model.addAttribute("user", user);
	...
	public String addPurchase( @ModelAttribute("purchase") Purchase purchase , 
		...
		System.out.println("addPurchase??? : " + event.getProduct());
	...
	public String getPurchaseList( @ModelAttribute("search") Search search , 
		...
		for ( int i = 0; i < list.size(); i++) {
			...
			if ( listEvent.getStartDate() != null ) {
				Calendar today = Calendar.getInstance();
				Date now = new Date(today.getTimeInMillis());
				
				int result = now.compareTo(listEvent.getStartDate());
				
				if ( result >= 0 ) {
					listEvent.getProduct().setPrice( (int) ( listEvent.getDcPersent() * listEvent.getProduct().getCostPrice() ) );
				}
				else {
					listEvent.getProduct().setPrice( listEvent.getProduct().getCostPrice() );
				}
	...
	public String getWishPurchase( @RequestParam("tranNo") int tranNo , 
		...
		Event event = productService.getProduct(purchase.getPurchaseProd().getProdNo());
